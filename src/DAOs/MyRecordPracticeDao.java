 ...
 * @author User
 */
public class MyRecordPracticeDao extends MySqlDao implements RecordDaoInterface
{
    ...
    public Map<Integer, String> getAnswerList(int recordId)
    {
        ...
        {
            conn = this.getConnection();
            String query = "SELECT `record_id`, `question_id`, `answer` FROM `practice_result` WHERE record_id = ?";
            ps = conn.prepareStatement(query);
            ps.setInt(1, recordId);
            rs = ps.executeQuery();
            while(rs.next())
